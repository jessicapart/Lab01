...
The Crown Estate at Windsor Great Park
```{r Q3}
...
50
```{r Ex_f}
visitors %>% filter(n_2022 >= 5e04, n_2022 <= 1e05) %>%count()
```
...
Not available, because there is no data provided for some attractions in 2021 
...
The data is not evenly spread across the settings but instead a few large values have skewed the value of the mean so that it is much larger than the median of the data. 
